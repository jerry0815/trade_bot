...
When the recommendation is **out of TQQQ** (~30% of the time), the report also suggests a **[defensive rotation](docs/strategies/defensive-rotation.md)** — hold the strongest of KMLM / TLT / GLD / SHY by 126-day momentum instead of cash. A proxy-extended backtest (assets spliced onto validated longer-history proxies) shows it adds a **modest ~+3 to +6pp CAGR vs actually holding SGOV** during out-of-market periods, but the drawdown benefit is *not* robust (the −49%→−41% cut was a 2022-specific effect that 2008 didn't repeat) and the managed-futures leg rests on a weak proxy. So it's a small return sweetener, kept as a **live suggestion** rather than a production default — the headline numbers above (cash-modeled) slightly *understate* returns, but the strategy stands on the trend rule + trailing stop.
...
| **Defensive rotation** (out-of-market sleeve) | Best-momentum of KMLM/TLT/GLD/SHY beats **holding SGOV** by ~+3 to +6pp CAGR while out of TQQQ (proxy-extended to 2000/2007), but the drawdown benefit isn't robust and the managed-futures proxy is weak. A modest return sweetener; live suggestion, not a production default. | [defensive-rotation](docs/strategies/defensive-rotation.md) |
